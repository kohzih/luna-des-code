...
void	DrawJIKI::HnbAttack()
{
	...
	switch(Wpd.flag&_HNB_LVCHK){	//装備のレベルを調べている
		case _HNB1_ON:
			if(!HNB1.WTamaCnt){
				//花火が指定された数発射待機状態になっているか調べる
				for(i=0;i<60;i++){
					if(KoDama[i]==HNB1.TTama[0])	ret=TRUE;
				}
				for(i=0;i<2;i++){
					if(ODama[i]==HNB1.TTama[1])		ret=TRUE;
				}
		...
		//----ここから弾の移動処理---------------------------------------------
			...
			for(i=0;(DWORD)i<HNB1.TTama[0];i++){		
				...
				if(HNB1.tm2[0][i].fTama&_TON){
					...
					//小玉の移動量決定
					if(!(HNB1.tm2[0][i].fTama&_TSAKU_ON)){//大玉の中で移動中
