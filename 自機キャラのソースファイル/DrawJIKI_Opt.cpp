...
BOOL	DrawJIKI::InitOpt()
{
	...
	for(i=0;i<4;i++){
		...
		if(!OPT.AtkDat[i].Opt_LSR1.tm)		return	FALSE;
		//確保した領域の先頭アドレスをコピー
	...
	for(i=0;i<4;i++){
		...
		if(!OPT.AtkDat[i].Opt_MSR1.tm)		return	FALSE;
		//確保した領域の先頭アドレスをコピー
...
{
	switch(Wpd.flag&_OPT_LVCHK){	//オプションのレベルを調べている
		case _OPT1_ON:
			OPT.OptLV=1;
			break;
		case _OPT2_ON:
		...
		for(i=0;i<OPT.OptLV;i++){
			switch(OPT.Jewel[i].fOPT&_fOPT_CHK){//固定、回転、追尾のどのフラクが立っているかチェック
				case _OPTKO_ON:
					...
					//オプションの現在の位置を初期化
					if(kotsu==0){	//固定フラグが立っている1つ目のオプションの初期座標
						OPT.Jewel[i].dCdn.x=(double)Cdn.x-40;
						OPT.Jewel[i].dCdn.y=(double)Cdn.y;
					}else if(kotsu==1){	//固定フラグが立っている2つ目のオプションの初期座標
						OPT.Jewel[i].dCdn.x=(double)Cdn.x+40;
						OPT.Jewel[i].dCdn.y=(double)Cdn.y;
					}else if(kotsu==2){	//固定フラグが立っている3つ目のオプションの初期座標
						OPT.Jewel[i].dCdn.x=(double)Cdn.x-20;
						OPT.Jewel[i].dCdn.y=(double)Cdn.y-40;
					}else if(kotsu==3){	//固定フラグが立っている4つ目のオプションの初期座標
						OPT.Jewel[i].dCdn.x=(double)Cdn.x+20;
						OPT.Jewel[i].dCdn.y=(double)Cdn.y-40;
					}
					kotsu++;
...
void	DrawJIKI::OptMovCnt(BYTE joyst)
{
	...
	{				//自機の現在の座標を取得
		...
		OPT.TuibCnt++;
		if(OPT.TuibCnt>_TUSIZE-1)	OPT.TuibCnt=0;
	}
	
	//固定オプション用
	OPT.KoteCdn[OPT.KoteCnt]=Cdn;	
	OPT.KoteCnt++;
	if(OPT.KoteCnt>_KOSIZE-1)	OPT.KoteCnt=0;
}
...
{	
	...
	for(short i=0;i<OPT.OptLV;i++){
		switch(OPT.Jewel[i].fOPT&_fOPT_CHK){	//移動パターンのチェック
			case _OPTKO_ON:						//固定
				kc=OPT.KoteCnt-(_KOSIZE-1);
				if(kc<0)	kc=kc+_KOSIZE;		//0～_KOSIZEをループさせる処理
				//移動させる処理
				if(kotsu==0){
			...
			case _OPTKA_ON:						//回転
				OPT.Jewel[i].dCdn.y=(double)Cdn.y;
				if(!OPT.KaitenF[kaisu]){		//回転方向フラグをチェック		
					//自機の左側で回転
					...
					if(OPT.KaitKaku[kaisu]>360){	//360より大きくならないようする		
			...
			case _OPTTU_ON:						//追尾
				...
				if(tc<0)	tc=tc+_TUSIZE;		//0～_TUSIZEをループさせる処理			
				//移動させる処理
...
{	
	...
	switch(Wpd.flag&_MAINWP_CHK){
		case _LSR_ON:
			switch (Wpd.flag&_LSR_LVCHK){//レーザーのレベルを調べている
				case _LSR1_ON:
					switch(OPT.OptLV){//オプションが何個ついているか調べている
						case 4:
...
void	DrawJIKI::OptAttackAtari()
{
	...
	switch (Wpd.flag&_LSR_LVCHK){
		case _LSR1_ON:
			for(i=0;i<OPT.OptLV;i++){	//現在装備しているオプションの数だけループ
				for(j=0;(DWORD)j<OPT.AtkDat[i].Opt_LSR1.TTama;j++){
					//未発射の弾とは当たり判定をしない
					if(OPT.AtkDat[i].Opt_LSR1.tm[j].fTama&_TON){
