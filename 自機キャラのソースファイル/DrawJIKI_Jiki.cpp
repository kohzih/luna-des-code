...
BOOL DrawJIKI::Atari(RECT JIKI)
{
	if (JIKI.top > SP_Cdn.y + (SP_Height - 4))
		...
		;
	else if (JIKI.bottom > SP_Cdn.y - 4)
		return FALSE;
...
void DrawJIKI::RendPWUP()
{
	...
	if ((JoyNew ^ JoyOld) & (JoyNew & JOY_UP))
	{
		...
		case 84:
			if (CurCdn.y == 100)
				CurCdn.y = 450; // MAIN(カーソルの位置)
			else if (CurCdn.y == 188)
				CurCdn.y = 100; // SUB
			else if (CurCdn.y == 276)
				CurCdn.y = 188; // OPT
			else if (CurCdn.y == 450)
				CurCdn.y = 276; // EXIT
			break;
		case 184:
			if (CurCdn.y == 100)
				CurCdn.y = 124; // LSR
			else if (CurCdn.y == 124)
				CurCdn.y = 100; // MSR
			else if (CurCdn.y == 188)
				CurCdn.y = 212; // HMM
			else if (CurCdn.y == 212)
				CurCdn.y = 188; // HNB
			else if (CurCdn.y == 308)
				CurCdn.y = 380; // OPT1
			else if (CurCdn.y == 332)
				CurCdn.y = 308; // OPT2
			else if (CurCdn.y == 356)
				CurCdn.y = 332; // OPT3
			else if (CurCdn.y == 380)
				CurCdn.y = 356; // OPT4
			break;
		default:
			break;
	...
	{
		if (CurCdn.x == 184 && CurCdn.y == 308)
			i = 0; // OPT1の位置
		else if (CurCdn.x == 184 && CurCdn.y == 332)
			i = 1; // OPT2の位置
		else if (CurCdn.x == 184 && CurCdn.y == 356)
			i = 2; // OPT3の位置
		else if (CurCdn.x == 184 && CurCdn.y == 380)
			i = 3; // OPT4の位置
		else
			i = 4;
			...
			if (CurCdn.x == 184)
			{
				...
				case 100: // LSRゲージアップ
					if (Wpd.LSR_temp < 320)
					{
						...
						Wpd.LSR_temp++;
						if (Wpd.LSR_temp >= 64)
							Wpd.flag |= _LSR1_ON; //レーザー1が使用可能になる
					}
					break;
				case 124: // MSRゲージアップ
					if (Wpd.MSR_temp < 320)
					{
						...
						Wpd.MSR_temp++;
						if (Wpd.MSR_temp >= 64)
							Wpd.flag |= _MSR1_ON; //レーザー1が使用可能になる
					}
					break;
				case 188: // HMMゲージアップ
					if (Wpd.HMM_temp < 320)
					{
						...
						Wpd.HMM_temp++;
						if (Wpd.HMM_temp >= 64)
							Wpd.flag |= _HMM1_ON; //レーザー1が使用可能になる
					}
					break;
				case 212: // HNBゲージアップ
					if (Wpd.HNB_temp < 320)
					{
						...
						Wpd.HNB_temp++;
						if (Wpd.HNB_temp >= 64)
							Wpd.flag |= _HNB1_ON; //レーザー1が使用可能になる
					}
					break;
				default:
					break;
		...
		if (CurCdn.x == 184)
		{
			...
			case 100: // LSRゲージダウン
				if (Wpd.LSR_temp > Wpd.LSR)
				{
					...
					Wpd.LSR_temp--;
					if (Wpd.LSR_temp < 64)
						Wpd.flag &= _LSR1_OFF; //レーザー1が使用不能になる
				}
				break;
			case 124: // MSRゲージダウン
				if (Wpd.MSR_temp > Wpd.MSR)
				{
					...
					Wpd.MSR_temp--;
					if (Wpd.MSR_temp < 64)
						Wpd.flag &= _MSR1_OFF; //ミサイル1が使用不能になる
				}
				break;
			case 188: // HMMゲージダウン
				if (Wpd.HMM_temp > Wpd.HMM)
				{
					...
					Wpd.HMM_temp--;
					if (Wpd.HMM_temp < 64)
						Wpd.flag &= _HMM1_OFF; //ホーミング1が使用不能になる
				}
				break;
			case 212: // HNBゲージダウン
				if (Wpd.HNB_temp > Wpd.HNB)
				{
					...
					Wpd.HNB_temp--;
					if (Wpd.HNB_temp < 64)
						Wpd.flag &= _HNB1_OFF; //花火1が使用不能になる
				}
				break;
			default:
				break;
	...
	{
		//カーソルがMAINの位置にある場合
		if (CurCdn.x == 84 && CurCdn.y == 100)
			CurCdn.x = 184; // LSRの位置
		else if (CurCdn.x == 184 && CurCdn.y == 100)
		{ // LSR
			Wpd.flag |= _LSR_ON;
			Wpd.flag &= _MSR_OFF;
		}
		else if (CurCdn.x == 184 && CurCdn.y == 124)
		{ // MSR
			Wpd.flag |= _MSR_ON;
			Wpd.flag &= _LSR_OFF;
		}
		//カーソルがSUBの位置にある場合
		else if (CurCdn.x == 84 && CurCdn.y == 188)
			CurCdn.x = 184; // HMMの位置
		else if (CurCdn.x == 184 && CurCdn.y == 188)
		{ // HMM
			Wpd.flag |= _HMM_ON;
			Wpd.flag &= _HNB_OFF;
		}
		else if (CurCdn.x == 184 && CurCdn.y == 212)
		{ // HNB
			Wpd.flag |= _HNB_ON;
			Wpd.flag &= _HMM_OFF;
		}
		...
		else if (CurCdn.x == 84 && CurCdn.y == 276)
		{
			...
			case 1:					  //レベル１
				...
				if (!(OPT.Jewel[0].fOPT & _fOPT_CHK))
					OPT.Jewel[0].fOPT |= _OPTKO_ON;
	...
	if ((JoyNew ^ JoyOld) & (JoyNew & JOY_B))
	{
		...
		else if (CurCdn.x >= 280)
			CurCdn.x = 184; // LSR,MSR,HMM,HNBゲージの位置
	}
	...
	{
	case _OPT4_ON:
		if (OPT.Jewel[3].fOPT & _OPTKO_ON)
		{
			x = 296;
			y = 380;
		}
		else if (OPT.Jewel[3].fOPT & _OPTKA_ON)
		{
			x = 360;
			y = 380;
		}
		else if (OPT.Jewel[3].fOPT & _OPTTU_ON)
		{
			x = 422;
			y = 380;
		}
	...
	case _OPT3_ON:
		if (OPT.Jewel[2].fOPT & _OPTKO_ON)
		{
			x = 296;
			y = 356;
		}
		else if (OPT.Jewel[2].fOPT & _OPTKA_ON)
		{
			x = 360;
			y = 356;
		}
		else if (OPT.Jewel[2].fOPT & _OPTTU_ON)
		{
			x = 422;
			y = 356;
		}
		lpDDSBack->BltFast(x, y, lpDDS_JIKI, &OPTWAKU, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
	case _OPT2_ON:
		if (OPT.Jewel[1].fOPT & _OPTKO_ON)
		{
			x = 296;
			y = 332;
		}
		else if (OPT.Jewel[1].fOPT & _OPTKA_ON)
		{
			x = 360;
			y = 332;
		}
		else if (OPT.Jewel[1].fOPT & _OPTTU_ON)
		{
			x = 422;
			y = 332;
		}
		lpDDSBack->BltFast(x, y, lpDDS_JIKI, &OPTWAKU, DDBLTFAST_WAIT | DDBLTFAST_SRCCOLORKEY);
	case _OPT1_ON:
		if (OPT.Jewel[0].fOPT & _OPTKO_ON)
		{
			x = 296;
			y = 308;
		}
		else if (OPT.Jewel[0].fOPT & _OPTKA_ON)
		{
			x = 360;
			y = 308;
		}
		else if (OPT.Jewel[0].fOPT & _OPTTU_ON)
		{
			x = 422;
			y = 308;
		}
