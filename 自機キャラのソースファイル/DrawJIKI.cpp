...
void	DrawJIKI::MoveJIKI()
{
	...
	if(!(fJIKI&_END_ON)){
		...
		//自機の移動
		if(JoyNew & JOY_UP){		//上が押されている
			Cdn.y -=Mov.y;				//移動量を加える					
		}else if(JoyNew & JOY_DOWN){//下が押されている
			Cdn.y +=Mov.y;								
		}
		if(JoyNew & JOY_RIGHT){		//右が押されている
			Cdn.x +=Mov.x;								
		}else if(JoyNew & JOY_LEFT){//左が押されている
			Cdn.x -=Mov.x;							
		}
		...
		if(JoyNew&JOY_A && !(fJIKI&_FUEL_ON)){
			...
			switch(Wpd.flag&_SUBWP_CHK){//現在選択されているサブウェポンの発射カウントを進める
				case _HMM_ON:
						HmmCnt();	//ホーミングミサイル
						break;
				case _HNB_ON:
			...
			if(fJIKI&_OPT_ON)	OptAttackCnt();		//オプションの弾発射のカウントを進める
			//Aが押された瞬間であった場合アーム発射フラグをオン
			if((JoyNew^JoyOld)&(JoyNew&JOY_A))	fJIKI |=_ARM_ON;
		}//Aが離された瞬間各攻撃用カウンタをリセット
		else if((JoyNew^JoyOld)&(JoyOld&JOY_A)){
			//弾が画面に存在しなくなるまで_WEPN_ONフラグは解除しない
			MainWepnCntReset();			//主砲発射までのカウントリセット
			switch(Wpd.flag&_SUBWP_CHK){//現在選択されているサブウェポンの発射カウントをリセット
				case _HMM_ON:
			...
			if(fJIKI&_OPT_ON)	OptAttackCntReset();//オプションの弾発射のカウントをリセット
		}
		//弾吸収燃料がある時にBが押された瞬間
		...
		//弾吸収中フラグが立っている時にBが押されっぱなしである
			...
			if(0 > FUEL_R-_FUELKY){	//FUEL値が0以下ならば
				FUEL_R=0;				//FUEL値0
	...
	//自機画面座標の補正
	if(Cdn.x<0)						Cdn.x=0;
	else if(Cdn.x+_JIKI_W>WinXMax)	Cdn.x=WinXMax-_JIKI_W;
	if(Cdn.y<0)						Cdn.y=0;
	else if(Cdn.y+32>WinYMax)		Cdn.y=WinYMax-32;
...
{		
	//ステージフラグに何かフラグが立っている場合リターン
	if(fSTAGE&_fSTAGE_CHK)	return	SP_fSPChr;
	//自機状態フラグにゲーム一時停止フラグが立っている場合リターン
	if(fJIKI&_GAME_STOP_ON)	return SP_fSPChr;
	//自機に撃墜フラグの立っている場合は当たり判定を行わない
	if(fJIKI&_END_ON)		return SP_fSPChr;
	...
	JikiTekitamaAtari();
	if(fJIKI&_END_ON)	return	SP_fSPChr;	//自機が撃墜された場合リターン
	
	//敵キャラの表示フラグが立っていない場合はリターン	
	if(!(SP_fSPChr&_SPON))		return	SP_fSPChr;
	else if(SP_fSPChr&_SPDES)	return	SP_fSPChr;
	//自機フラグに敵キャラ消滅が立っている場合画面上の敵キャラに一定のダメージ
	else if(fJIKI&_PWUPD_ON){
		JikiTekiDmg(_PWUPDDMG);
		//敵キャラの破壊フラグが立っている場合はリターン
		if(SP_fSPChr&_SPDES)	return	SP_fSPChr;
	}
...
void	DrawJIKI::RendJIKI()
{
	...
	if(fJIKI&_END_ON){
		...
		//------------- オプションが装備されている時の転送 -------------
		if(fJIKI&_OPT_ON)	OptRend();
		
		//自機が無敵状態の時に点滅させる処理
		if(fJIKI&_INVINCIBLE_ON && !(fJIKI&_GAME_STOP_ON)){
			if(!(InvincCnt--)){	//無敵時間を減らす
				InvincCnt=0;
		...
		//-------------- 画面に弾が存在している時の転送 ----------------
		if(fJIKI&_WEPN_ON)		MainWepnRend();	//主砲の弾の描画
		if(fJIKI&_SUBHMM_ON)	HmmRend();		//ホーミングミサイルの描画
		if(fJIKI&_SUBHNB_ON)	HnbRend();		//花火の描画処理
		if(fJIKI&_OPTWEP_ON)	OptWepRend();	//オプションの発射した弾の描画
	}
