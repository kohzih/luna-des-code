...
void	DrawJIKI::HmmMovCnt()
{
	...
	for(int i=0;(DWORD)i<HMM1.TTama;i++){
		...
		if(HMM1.tm[i].LockTekiNum==SP_Num && HMM1.tm[i].STamaCnt==0){
			...
			if(X==0){ 
				if(Y<0)			HMM1.tm[i].TekiKakudo=90;	
				else if(Y>0)	HMM1.tm[i].TekiKakudo=270;
			}else if(Y==0){
				if(X<0)			HMM1.tm[i].TekiKakudo=180;
				else if(X>0)	HMM1.tm[i].TekiKakudo=0;
			}else{
				//敵キャラと弾との角度を計算
				x=X<0 ? -X:X;
				y=Y<0 ? -Y:Y;
				...
				if(X<0){
					if(Y<0)			HMM1.tm[i].TekiKakudo=180-HMM1.tm[i].TekiKakudo;//第2象限
					else if(Y>0)	HMM1.tm[i].TekiKakudo=180+HMM1.tm[i].TekiKakudo;//第3象限
				}else if(X>0){
					if(Y>0)			HMM1.tm[i].TekiKakudo=360-HMM1.tm[i].TekiKakudo;//第4象限
				}
			}	
			//弾の移動量を計算(距離の近い方向で回転)
			if(HMM1.tm[i].Kakudo>HMM1.tm[i].TekiKakudo){
				if(HMM1.tm[i].Kakudo-HMM1.tm[i].TekiKakudo>180){//反時計周り
					HMM1.tm[i].Kakudo +=15;
				}else{											//時計周り
					HMM1.tm[i].Kakudo -=15;
				}
			}else if(HMM1.tm[i].Kakudo<HMM1.tm[i].TekiKakudo){
				if(HMM1.tm[i].TekiKakudo-HMM1.tm[i].Kakudo>180){//時計周り
					HMM1.tm[i].Kakudo -=15;
				}else{											//反時計周り
					HMM1.tm[i].Kakudo +=15;
				}
			...
			//角度の補正(0-345度に限定する処理)
			if(HMM1.tm[i].Kakudo==360)		HMM1.tm[i].Kakudo=0;
			else if(HMM1.tm[i].Kakudo>360)	HMM1.tm[i].Kakudo=15;
			else if(HMM1.tm[i].Kakudo<0)	HMM1.tm[i].Kakudo=345;
...
void	DrawJIKI::HmmAttack()
{
	...
	switch(Wpd.flag&_HMM_LVCHK){	//装備のレベルを調べている
		case _HMM1_ON:
			if(!HMM1.WTamaCnt){
			...
			for(i=0;(DWORD)i<HMM1.TTama;i++){	
				...
				if(HMM1.tm[i].fTama&_TON)
				{
					...
					if(!(HMM1.tm[i].STamaCnt))	HMM1.tm[i].STamaCnt=HMM1.tm[i].STama;//カウンタを再セット
