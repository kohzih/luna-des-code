...
BOOL	FaInOut::LittleBlack(int BLACK,bool flag)
{
	...
	if(!flag && !LBFadeF){
		...
		for(int i=StartPal;i<StartPal+PalEntry-1;i++)
		{
			...
			if(apeH3[i].r>>16 <= 0){ //減色した結果要素が0より小さくなった場合、0をセット
				apeH3[i].r=0;
				apeH2[i].peRed=0;
			}
			else	apeH2[i].peRed=apeH3[i].r>>16;
			//緑の要素を減色
			...
			else	apeH2[i].peGreen=apeH3[i].g>>16;
			//青の要素を減色
