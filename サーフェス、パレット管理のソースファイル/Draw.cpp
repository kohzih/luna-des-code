...
BOOL	Draw::RestoreJIKI()
{
	...
	switch(StageNum){
		...
		case 1:	
			break;
		case 2:
			if(!lpDDS_JIKI)		return	FALSE;
	
			ret=lpDDS_JIKI->IsLost();
			if(ret==DD_OK)		break;					//サーフェスがロストしていないかチェック
...
BOOL	Draw::LoadBitMap(char *FileName,LPDIRECTDRAWSURFACE7 &lpDDsuf)
{
	...
	if(lpDDsuf == NULL)
	{
		...
		if(fFullscreen)	ddsd.ddsCaps.dwCaps=DDSCAPS_OFFSCREENPLAIN;
...
//読み込んだビットマップをオフスクリーンサーフェスに登録
	...
	if(bm.bmHeight>0)
	{
		...
		{	
			...
			{
				if(*lpSrc)	*lpDst=*lpSrc+10;		//パレットを１０進めた状態で登録	
				else		*lpDst=*lpSrc;			//０の時は透明色のためそのまま登録
				lpDst++;
				lpSrc++;
			}	
			lpDst -=ddsd.lPitch;
