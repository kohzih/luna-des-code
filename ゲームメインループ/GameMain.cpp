...
{
	//プライマリサーフェスロストチェック
	if(!lpDDSPrimary==NULL && lpDDSPrimary->IsLost()==DDERR_SURFACELOST)	lpDDSPrimary->Restore();
	//バックサーフェスクリア
	DDBLTFX			ddbltfx;
	...
	//ゲーム終了フラグのチェック
	if(lpDraw->GetfSTAGE()&_GAME_EXIT)	return	FALSE;
	...
	lpBG->RendBG();												//背景の描画
	for(i=0;(DWORD)i<lpDraw->EventCnt;i++)	lpSP[i].RendSP();	//敵キャラの描画
