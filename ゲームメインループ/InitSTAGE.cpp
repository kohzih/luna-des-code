...
//				ret=lpFInOut->BlackIn(0x01020000);
				if(ret)	lpDraw->SetfSTAGE(_STAGE_START_OFF,0);
				break;
...
//				if(WaitCnt++>2){
		...
		case 1://タイトル画面
			if(!(lpDraw->GetfSTAGE()&_fSTAGE_CHK_InitSTAGE)){
				...
				JoyOld=JoyNew;
				JoyNew=Joy_Get();
				if((JoyNew^JoyOld)&(JoyNew&JOY_UP)){	//上が押された
					if(!(lpBG->GetfBG()&_SetBG00_ON))	lpBG->SetfBG(_SetBG00_ON,1);
					else								lpBG->SetfBG(_SetBG00_OFF,0);
				}else if((JoyNew^JoyOld)&(JoyNew&JOY_DOWN)){//下が押された
					if(!(lpBG->GetfBG()&_SetBG00_ON))	lpBG->SetfBG(_SetBG00_ON,1);
					else								lpBG->SetfBG(_SetBG00_OFF,0);								
				}
				//Aが押すとゲームスタート
				if((JoyNew^JoyOld)&(JoyNew&JOY_A)){
					if(lpBG->GetfBG()&_SetBG00_ON)	lpDraw->SetfSTAGE(_GAME_EXIT,1);
					else							lpDraw->SetfSTAGE(_STAGE_ENDWAIT_ON,1);
				}
			}
			break;
		...
		case 2:	//ステージ1
			...
			if(!(lpDraw->GetfSTAGE()&_fSTAGE_CHK_InitSTAGE) && lpJIKI->GetfJIKI()&_GAME_STOP_ON){
				...
				if(lpJIKI->GetfJIKI()&_END_ON){
					...
					if(!ret){
						//BGMを一時停止させる
						if(lpJIKI->GetfJIKI()&_BOSS_ON){//ボス戦
