...
void BOSS01::Attack1Boss01(POINT j_cdn)
{
	...
	if (!B_Msr.WTamaCnt--)
	{
		...
		for (short i = 0; i < 7; i++)
		{ // 7発分発射可能な弾が確保されているかチェック
			if (AttackTama1[i] >= B_Msr.TTama)
				ret = FALSE;
		}
		...
		if (ret)
		{
			...
			else
			{
				...
				{
					if (Y < 0)
						Kakudo = 180 - Kakudo; //第2象限
					else if (Y > 0)
						Kakudo = 180 + Kakudo; //第3象限
				}
				else if (X > 0)
				{
					if (Y > 0)
						Kakudo = 360 - Kakudo; //第4象限
				}
			}
