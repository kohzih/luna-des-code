...
BYTE	Joy_Get(void)
{
	...
	if(fInputDevice)
	{
		...
		if(js.lY<0 || KEYDOWN(buffer, DIK_UP))		Joy|=JOY_UP;
		if(js.lY>0 || KEYDOWN(buffer, DIK_DOWN))	Joy|=JOY_DOWN;
		if(js.lX<0 || KEYDOWN(buffer, DIK_LEFT))	Joy|=JOY_LEFT;
		if(js.lX>0 || KEYDOWN(buffer, DIK_RIGHT))	Joy|=JOY_RIGHT;
		if(js.rgbButtons[0] & 0x80	|| KEYDOWN(buffer, DIK_Z))		Joy|=JOY_A;
		if(js.rgbButtons[1] & 0x80 	|| KEYDOWN(buffer, DIK_X))		Joy|=JOY_B;
	}else{
	//ジョイスティックが繋がっていない場合の処理
		lpDIKeyboard->Poll();
		lpDIKeyboard->GetDeviceState(sizeof(buffer),(LPVOID)&buffer);
	
		if(KEYDOWN(buffer, DIK_UP))		Joy|=JOY_UP;
		if(KEYDOWN(buffer, DIK_DOWN))	Joy|=JOY_DOWN;
		if(KEYDOWN(buffer, DIK_LEFT))	Joy|=JOY_LEFT;
		if(KEYDOWN(buffer, DIK_RIGHT))	Joy|=JOY_RIGHT;
		if(KEYDOWN(buffer, DIK_Z))		Joy|=JOY_A;
		if(KEYDOWN(buffer, DIK_X))		Joy|=JOY_B;
	}
