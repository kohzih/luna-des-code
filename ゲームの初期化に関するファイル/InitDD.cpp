...
BOOL	InitDD()
{
	...
	ret2=lpDD->EnumDisplayModes(	DDEDM_REFRESHRATES,
									NULL,(LPVOID)&ret,
									EnumModesCallback2);
	if(ret2==DDERR_INVALIDOBJECT)		MessageBox(	hWnd,"DDERR_INVALIDOBJECT","ret2",MB_OK|MB_APPLMODAL);
	if(ret2==DDERR_INVALIDPARAMS)		MessageBox(	hWnd,"DDERR_INVALIDPARAMS","ret2",MB_OK|MB_APPLMODAL);
	...
	else
	{
		...
		if(ret != DD_OK)		
		{
			...
			if(ret==DDERR_INCOMPATIBLEPRIMARY)	 MessageBox(hWnd,"A","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_INVALIDCAPS)	 MessageBox(hWnd,"B","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_INVALIDOBJECT)	 MessageBox(hWnd,"C","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_INVALIDPARAMS)	 MessageBox(hWnd,"D","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_INVALIDPIXELFORMAT)	 MessageBox(hWnd,"E","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NOALPHAHW)	 MessageBox(hWnd,"F","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NOCOOPERATIVELEVELSET)	 MessageBox(hWnd,"G","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NODIRECTDRAWHW)	 MessageBox(hWnd,"H","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NOEMULATION)	 MessageBox(hWnd,"I","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NOEXCLUSIVEMODE)	 MessageBox(hWnd,"J","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NOFLIPHW)	 MessageBox(hWnd,"K","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NOMIPMAPHW)	 MessageBox(hWnd,"L","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NOOVERLAYHW)	 MessageBox(hWnd,"M","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_NOZBUFFERHW)	 MessageBox(hWnd,"N","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_OUTOFMEMORY)	 MessageBox(hWnd,"O","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_OUTOFVIDEOMEMORY)	 MessageBox(hWnd,"P","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_PRIMARYSURFACEALREADYEXISTS)	 MessageBox(hWnd,"Q","エラー",MB_OK|MB_APPLMODAL);
			if(ret==DDERR_UNSUPPORTEDMODE)	 MessageBox(hWnd,"R","エラー",MB_OK|MB_APPLMODAL);
